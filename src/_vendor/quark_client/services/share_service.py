...
class ShareService:
    ...
    def create_share(
        ...
        Args:
            file_ids: 文件ID列表
            title: 分享标题
            expire_days: 过期天数，0表示永久（兼容项）
            password: 提取码，None表示无密码
            expired_at_ms: 精确到毫秒的过期时间戳，优先于 expire_days
        ...
        data = {
            ...
            'expired_type': 1 if expire_days == 0 and expired_at_ms is None else 2  # 1=永久，2=有期限
        }

        # 如果设置了过期时间，添加过期时间字段
        if expired_at_ms is not None:
            data['expired_at'] = int(expired_at_ms)
        elif expire_days > 0:
            import time
