...
- **🖼️ 高级功能支持**：
    ...
    - **帷幕合成**：使用 `拉上帷幕` 设置会话表图，后续出图可生成幻影坦克效果。
    - **按配置导出**：可在通用设置中开启发送二维码和夸克分享链接，也可用 `QR=true` / `QK=true` 对单次请求开启；二维码使用独立的匿名图床链接，每张图片只发送一张二维码，不需要夸克登录。
    - **Anlas 审计**：管理员可使用 `nai账户` 查询余额，使用 `nai审计` 查看本地消耗记录。
- **⚙️ 强大的系统管理**：
...
| `general.send_quark_link` | 出图后发送夸克网盘分享链接，默认关闭。 | ❌ 否 |
| `general.send_qr` | 出图后发送图片二维码，默认关闭。 | ❌ 否 |
| `reaction.*` | 出图后随机短评和表情包反馈。 | ❌ 否 |
| `curtain.*` | 帷幕合成参数。 | ❌ 否 |
